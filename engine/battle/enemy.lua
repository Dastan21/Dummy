...
function Enemy:getCheckText()
  ...
  check = check .. Lang.translate("BATTLE_STAT_AT") .. " " .. self:getAT() .. " "
  check = check .. Lang.translate("BATTLE_STAT_DF") .. " " .. self:getDF()
...
function Enemy:new(name, sprite)
  ...
  assert(Battle.getEncounter() ~= nil, "Cannot add an enemy outside of a battle")
  self.encounter = Battle.getEncounter()
  self.encounter:addEnemy(self)
