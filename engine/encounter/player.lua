...
function self.load()
  ...
  self.hp_text = Text.new(Lang.translate("ENCOUNTER_STAT_HP"))
  self.hp_text:setPosition(244, 405)
  self.hp_text:setOrigin(0)
  self.hp_text:setFont(Font.FONT.WONDER)
...
function self.show()
  self.soul_sprite:setActive(true)
end

--- Hides the player's soul
function self.hide()
  self.soul_sprite:setActive(false)
end

--- Wether the player's soul is hidden
--- @return boolean
function self.isHidden()
  return not self.soul_sprite:isActive()
end
...
function self.setHP(hp)
  ...
  self.hp = math.clamp(hp, 1, math.min(self.max_hp, 99))
