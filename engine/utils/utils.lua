...
function modRequire(modname, altname)
  local mod = ModList.getCurrentMod()
  assert(mod ~= nil or altname ~= nil, "Cannot require outside of a mod")
  if mod ~= nil then
    local success, module = pcall(require, "mods." .. mod:getId() .. "." .. modname)
    if not success then
      if altname ~= nil then
