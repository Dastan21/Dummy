...
local Shader = Class("Dummy.Shader")
...
function Shader:new(shader)
  ...
  if type(shader) == "string" then
    ...
    local success, love_shader = pcall(love.graphics.newShader,
      "mods/" .. mod:getId() .. "/assets/shaders/" .. shader .. ".glsl")
    if not success then
      success, love_shader = pcall(love.graphics.newShader, "assets/shaders/" .. shader .. ".glsl")
      assert(success, "Shader \"" .. shader .. "\" not found: " .. tostring(love_shader))
    end

    self.shader = love_shader
  else
    self.shader = shader
  end
