...
function self.load(err)
  ...
  if err == "stack overflow" then
    self.traceback = "Stack overflow!"
  else
    self.traceback = debug.traceback(err)
  end
  self.traceback = Lang.translate("ERROR_SCENE_ERROR") .. " " .. self.traceback

  local _, wrapped_error = Font.FONT.MAIN_TEXT:getWrap(self.traceback, 630)
  self.error_text = Text.new(table.concat(wrapped_error, "\n"))
  self.error_text:setPosition(5, 5)
  self.error_text:setOrigin(0, 0)
  self.error_text:setFont(Font.FONT.MAIN_TEXT)

  local black_rectangle_draw = Drawable.new()
  black_rectangle_draw:setLayer(Constants.LAYERS.ABOVE_UI)
  black_rectangle_draw.draw = function()
  ...
  self.back_main_menu_text:setFont(Font.FONT.MAIN_TEXT)
  ...
  self.copy_traceback_text:setFont(Font.FONT.MAIN_TEXT)
